...
model_save_path = "/content/OCR-Example/checkpoints"
...
def test_model(model, test_ds, number_of_test_imgs: int = 10):
    ...
    beam_search_decoder = ctc_decoder(
        lexicon=None,
        tokens=test_ds.emnist_dataset.classes+['|'],
        nbest=3,
        beam_size=3, sil_token='|', blank_token='|'
        )
    beam_search_result  =[res[0].tokens for res in  beam_search_decoder(y_pred.detach().cpu())]
    ...
    for j in range(len(x_test)):
        ...
        plt.gcf().text(x=0.1, y=0.1, s="Actual: " + str([test_ds.emnist_dataset.classes[int(i)] for i in y_test[j].numpy() if i<47]))
        plt.gcf().text(x=0.1, y=0.2, s="Predicted: " + str([test_ds.emnist_dataset.classes[int(i)] for i in beam_search_result[j].numpy() if i<47]))
        plt.savefig(f"/content/OCR-Example/output/plot_{j}.png")
        plt.show()
...
if __name__ == "__main__":
    ...
    #model.load_state_dict(torch.load("/content/OCR-Example/checkpoints/checkpoint_5.pt"))
